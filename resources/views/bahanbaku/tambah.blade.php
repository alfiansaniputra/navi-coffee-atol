...
@endif
<form class="" action="{{route('admin.bahanbaku-tambah')}}" method="post" enctype="multipart/form-data">
    <div class="form-group mb-4">
        <label>Nama Bahan Baku</label>
        <input class="form-control" type="text"  placeholder="Nama bahanbaku" name="nama_bahan_baku" required>
    </div>
    <div class="form-group mb-4">
        <label>Jenis bahanbaku</label>
        <input class="form-control" type="text"  placeholder="jenis_bahan_baku" name="jenis_bahan_baku" required>
    </div>
    <div class="form-group mb-4">
        <label>Stok</label>
        <input class="form-control" type="number"  placeholder="Stok Bahan Baku" name="stok" required>
    </div>
        ...
        <div class="">
            <select name="id_suplier"  class="form-control" required>
                ...
                @foreach($suplier as $suplier)
                    <option name="id_suplier" value="{{ $suplier['id']}}" >{{ $suplier['nama_perusahaan']}}</option>
                @endforeach
            </select>
        </div>
    </div>
