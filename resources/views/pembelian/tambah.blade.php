...
@endif
<form class="" action="{{route('admin.resep-tambah')}}" method="post" enctype="multipart/form-data">
        ...
        <div class="">
            <select name="id_kopi"  class="form-control" required>
                ...
                @foreach($kopi as $kopi)
                    <option name="id_kopi" value="{{ $kopi['id']}}" >{{ $kopi['nama_kopi']}}</option>
                @endforeach
            </select>
        </div>
    </div>
        ...
        <div class="">
            <select name="id_bahan_baku"  class="form-control" required>
                ...
                @foreach($bahanbaku as $bahanbaku)
                    <option name="id_bahan_baku" value="{{ $bahanbaku['id']}}" >{{ $bahanbaku['nama_bahan_baku']}}</option>
                @endforeach
            </select>
        </div>
    </div>
    <div class="form-group mb-4">
        ...
        <input class="form-control" type="number"  placeholder="Qty (jumlah) / satuan gram" name="qty" required>
    </div>
